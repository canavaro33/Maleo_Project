...
router.post("/", verifyJWT, checkRole("super_admin", "admin"), validate(yearSchema), async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json({ message: "Tahun ajaran berhasil ditambahkan", data: year });
  } catch (error) {
    res.status(500).json({ message: "Terjadi kesalahan server" });
  }
});
...
router.put("/:id", verifyJWT, checkRole("super_admin", "admin"), validate(yearSchema.partial()), async (req: Request, res: Response) => {
  try {
    ...
    const year = await prisma.academicYear.update({ where: { id: Number(req.params.id) }, data });
    res.json({ message: "Tahun ajaran berhasil diperbarui", data: year });
  } catch (error: any) {
