...
const rpsOwnershipGuard = async (req: any, res: Response, next: any) => {
  try {
    const rpsId = Number(req.params.rpsId || req.params.id);
    if (!rpsId) return next(); // lewati jika tidak ada rpsId di param

    const rps = await prisma.rPS.findUnique({ where: { id: rpsId } });
    if (!rps) {
...
const uploadDir = path.join(process.cwd(), "uploads", "rps-materials");
...
router.get("/student/my-rps", async (req: any, res: Response) => {
  try {
...
router.post("/student/materials/:materialId/access", async (req: any, res: Response) => {
  try {
    ...
    await prisma.rPSMaterialAccess.upsert({
...
const meetings = Math.min(Math.max(Number(totalMeetings), 1), 16);
    ...
    const rps = await prisma.$transaction(async (tx) => {
    ...
    const rpsWithMeetings = await prisma.rPS.findUnique({
      where: { id: rps.id },
      include: {
...
router.get("/:id", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
...
router.put("/:id", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
...
router.put("/:id/publish", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
...
router.put("/:id/archive", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
...
router.delete("/:id", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
    await prisma.rPS.delete({ where: { id: Number(req.params.id) } });
...
router.get("/:id/bundle", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
    const rps = await prisma.rPS.findUnique({
      where: { id: Number(req.params.id) },
      include: {
    ...
    if (!rps) return res.status(404).json({ success: false, message: "RPS tidak ditemukan." });
...
router.put("/:rpsId/meetings/:meetingId", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
    const meetingId = Number(req.params.meetingId);
    const rpsId = Number(req.params.rpsId);

    // Pastikan meeting ada di RPS ini
    const meeting = await prisma.rPSMeeting.findFirst({
    ...
    const updated = await prisma.rPSMeeting.update({
      where: { id: meetingId },
...
router.put("/:rpsId/meetings/:meetingId/publish", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
    const meetingId = Number(req.params.meetingId);
    const rpsId = Number(req.params.rpsId);

    const meeting = await prisma.rPSMeeting.findFirst({ where: { id: meetingId, rpsId } });
    if (!meeting) {
      return res.status(404).json({ success: false, message: "Pertemuan tidak ditemukan." });
    }
...
router.post(
  "/:rpsId/meetings/:meetingId/materials/upload",
  teacherGuard,
  rpsOwnershipGuard,
  upload.single("file"),
  async (req: any, res: Response) => {
    try {
      ...
      const fileType = mimeToType[req.file.mimetype] || "pdf";
      const fileUrl = `/uploads/rps-materials/${req.file.filename}`;

      const material = await prisma.rPSMaterial.create({
        data: {
          meetingId,
...
router.post("/:rpsId/meetings/:meetingId/materials/link", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
...
router.delete("/:rpsId/meetings/:meetingId/materials/:materialId", teacherGuard, rpsOwnershipGuard, async (req: any, res: Response) => {
  try {
    const materialId = Number(req.params.materialId);
    const material = await prisma.rPSMaterial.findUnique({ where: { id: materialId } });
    ...
    await prisma.rPSMaterial.delete({ where: { id: materialId } });
...
router.post("/:id/import-excel", teacherGuard, rpsOwnershipGuard, upload.single("excel"), async (req: any, res: Response) => {
  try {
    if (!req.file) {
    ...
    for (const row of rows) {
      ...
      const meeting = await prisma.rPSMeeting.findFirst({ where: { rpsId, meetingNumber } });
      if (meeting) {
        await prisma.rPSMeeting.update({
          where: { id: meeting.id },
          data: {
