  ...
  try {
    ...
    const teacher = await prisma.teacher.findFirst({
      ...
      include: { 
        ...
        schedules: { select: { class: { select: { id: true, name: true } } }, distinct: ['classId'] }
      }
    });
...
export const trackAccess = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!student) return res.status(404).json({ success: false, message: "Profil siswa tidak ditemukan." });
    ...
    res.status(500).json({ success: false, message: "Gagal merekam akses." });
  }
};
