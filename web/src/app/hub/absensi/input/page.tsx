    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      {role === "teacher" ? (
        <>
          ...
          {successBanner && (
            <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
          ...
          {studentRecords.length > 0 ? (
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Card className="p-4 border-l-4 border-l-emerald-600 shadow-sm bg-emerald-50/30">
                  <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider mb-1">Hadir</p>
                  <h3 className="text-2xl font-bold text-emerald-900">{stats.hadir}</h3>
                </Card>
                <Card className="p-4 border-l-4 border-l-blue-600 shadow-sm bg-blue-50/30">
                  <p className="text-[10px] font-bold text-blue-700 uppercase tracking-wider mb-1">Izin</p>
                  <h3 className="text-2xl font-bold text-blue-900">{stats.izin}</h3>
                </Card>
                <Card className="p-4 border-l-4 border-l-amber-600 shadow-sm bg-amber-50/30">
                  <p className="text-[10px] font-bold text-amber-700 uppercase tracking-wider mb-1">Sakit</p>
                  <h3 className="text-2xl font-bold text-amber-900">{stats.sakit}</h3>
                </Card>
                <Card className="p-4 border-l-4 border-l-rose-600 shadow-sm bg-rose-50/30">
                  ...
                  <h3 className="text-2xl font-bold text-rose-900">{stats.alpa}</h3>
                </Card>
              </div>
                        ...
                        <tr key={record.studentId} className="hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <input
                              type="text"
                              placeholder="Catatan..."
                              className="w-full bg-card border border-border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                              value={record.note}
              ...
              <div className="flex justify-end pt-4">
                <Button 
                  ...
                  className="px-10 shadow-lg shadow-indigo-500/20"
                >
            ...
            <div className="py-24 text-center border-2 border-dashed border-border rounded-3xl bg-gradient-to-br from-card to-muted/20">
              ...
              <h3 className="text-2xl font-bold text-foreground">Mulai Input Absensi</h3>
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
          {/* Summary Cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="p-5 border-l-4 border-l-emerald-600 shadow-sm bg-emerald-50/30">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-1">Total Hadir</p>
                  <h3 className="text-3xl font-bold text-emerald-900">{studentStats.hadir}</h3>
                </div>
            ...
            <Card className="p-5 border-l-4 border-l-blue-600 shadow-sm bg-blue-50/30">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[11px] font-bold text-blue-700 uppercase tracking-wider mb-1">Total Izin</p>
                  <h3 className="text-3xl font-bold text-blue-900">{studentStats.izin}</h3>
                </div>
            ...
            <Card className="p-5 border-l-4 border-l-amber-600 shadow-sm bg-amber-50/30">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-[11px] font-bold text-amber-700 uppercase tracking-wider mb-1">Total Sakit</p>
                  <h3 className="text-3xl font-bold text-amber-900">{studentStats.sakit}</h3>
                </div>
            ...
            <Card className="p-5 border-l-4 border-l-rose-600 shadow-sm bg-rose-50/30">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-3xl font-bold text-rose-900">{studentStats.alpa}</h3>
                </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-foreground">Riwayat Kehadiran</h2>
            </div>
            
            {studentHistory.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {studentHistory.map((item, idx) => (
                  <Card key={idx} className="p-4 border-border hover:border-indigo-300 transition-all hover:shadow-md group">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className="p-2.5 rounded-xl bg-muted group-hover:bg-indigo-50 transition-colors">
                          <Calendar size={18} className="text-muted-foreground group-hover:text-indigo-600" />
                        </div>
                        <div>
              ...
              <div className="py-24 text-center border-2 border-dashed border-border rounded-3xl bg-gradient-to-br from-card to-muted/20">
                ...
                <h3 className="text-2xl font-bold text-foreground">Belum Ada Riwayat</h3>
                <p className="text-sm text-muted-foreground max-w-md mx-auto mt-3 leading-relaxed">
...
function StatusButton({ active, type, onClick }: { 
  ...
  const styles = {
    hadir: active ? "bg-emerald-100 text-emerald-700 border-emerald-300 shadow-sm" : "bg-transparent text-muted-foreground border-transparent hover:bg-emerald-50",
    izin: active ? "bg-blue-100 text-blue-700 border-blue-300 shadow-sm" : "bg-transparent text-muted-foreground border-transparent hover:bg-blue-50",
    sakit: active ? "bg-amber-100 text-amber-700 border-amber-300 shadow-sm" : "bg-transparent text-muted-foreground border-transparent hover:bg-amber-50",
