...
const STATUS_CONFIG: Record<string, { label: string; color: string; bgColor: string; icon: React.ElementType }> = {
  hadir: { label: "Hadir", color: "text-emerald-700", bgColor: "bg-emerald-100 border-emerald-200", icon: CheckCircle2 },
  izin: { label: "Izin", color: "text-blue-700", bgColor: "bg-blue-100 border-blue-200", icon: Clock },
  sakit: { label: "Sakit", color: "text-amber-700", bgColor: "bg-amber-100 border-amber-200", icon: AlertCircle },
  ...
  if (isTeacher) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Kehadiran Siswa</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Gunakan menu <strong>Input Absensi</strong> di sidebar untuk mencatat kehadiran siswa.
          </p>
        </div>
        <Card className="p-12 text-center border-dashed border-2 border-muted">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">Fitur Tersedia di Menu Lain</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Kehadiran Saya</h1>
        ...
        <>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-foreground">Persentase Kehadiran Bulan Ini</h3>
              </div>
              <span className={cn(
                "text-2xl font-black",
                data.presentRate >= 80 ? "text-emerald-600" :
                data.presentRate >= 60 ? "text-amber-600" : "text-rose-600"
              )}>
                {data.presentRate}%
              </span>
            </div>
            <div className="h-3 w-full bg-muted rounded-full overflow-hidden">
              <div
                className={cn(
                  ...
                  data.presentRate >= 80 ? "bg-emerald-500" :
                  data.presentRate >= 60 ? "bg-amber-500" : "bg-rose-500"
                )}
            ...
            <div className="flex justify-between mt-2 text-xs text-muted-foreground">
              ...
              <span className={cn(
                "font-medium",
                data.presentRate >= 80 ? "text-emerald-600" : data.presentRate >= 60 ? "text-amber-600" : "text-rose-600"
              )}>
          ...
          <Card className="overflow-hidden">
            <div className="p-5 border-b border-border flex items-center gap-2">
              ...
              <h3 className="font-bold text-foreground">Riwayat Kehadiran — {MONTHS[selectedMonth - 1]} {selectedYear}</h3>
