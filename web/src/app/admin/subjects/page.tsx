...
export default function SubjectsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="mb-6">
          <Input placeholder="Cari nama atau kode mapel..." icon={<Search size={16} />} value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left py-3 px-4 font-semibold text-muted-foreground">Nama Mapel</th>
                  <th className="text-left py-3 px-4 font-semibold text-muted-foreground">Guru Pengampu</th>
                  <th className="text-left py-3 px-4 font-semibold text-muted-foreground">Tingkat</th>
                ...
                {filtered.length > 0 ? (
                  filtered.map((subject, i) => (
                    <tr key={subject.id} className="border-b border-border/50 hover:bg-accent/50 transition-colors">
                      ...
                      <td className="py-3 px-4 font-mono text-xs font-semibold text-indigo-600">{subject.code}</td>
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openEdit(subject)} className="p-2 rounded-lg text-muted-foreground hover:text-indigo-600 hover:bg-indigo-50 transition-colors">
                            <Pencil size={16} />
                          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? <Loader2 className="animate-spin" size={16} /> : (editingSubject ? "Simpan Perubahan" : "Tambah Mapel")}
            </Button>
          </div>
        </form>
      </Modal>
