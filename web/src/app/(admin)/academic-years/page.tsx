  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Tahun Ajaran</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loading ? (
          ...
          years.map((year) => (
            <Card key={year.id} className={`relative overflow-hidden transition-all hover:shadow-md ${year.isActive ? "ring-2 ring-indigo-500 shadow-indigo-100" : ""}`}>
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-1">
                  {!year.isActive && (
                    <button onClick={() => handleSetActive(year)} className="p-2 rounded-lg text-muted-foreground hover:text-emerald-600 hover:bg-emerald-50 transition-colors" title="Set Aktif">
                      <CheckCircle2 size={18} />
                    </button>
                  )}
                  <button onClick={() => openEdit(year)} className="p-2 rounded-lg text-muted-foreground hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Edit">
                    <Pencil size={18} />
                  </button>
              ...
              {year.isActive && <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-500 to-purple-500" />}
            </Card>
          ))
        )}
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => !isSubmitting && setIsModalOpen(false)} title={editingYear ? "Edit Tahun Ajaran" : "Tambah Tahun Ajaran Baru"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg hover:bg-accent transition-colors">
            <input type="checkbox" checked={formData.isActive} onChange={e => setFormData({...formData, isActive: e.target.checked})} className="rounded border-border text-indigo-600" />
