  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTeacher) {
        ...
        await apiService.create("/teachers", formData);
        setSuccess("Guru baru berhasil ditambahkan");
      }
      await fetchData();
    ...
    <div className="space-y-6">
                  ...
                  <tr key={teacher.id} className="border-b border-border/50 hover:bg-accent/50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEdit(teacher)} className="p-2 rounded-lg text-muted-foreground hover:text-indigo-600 hover:bg-indigo-50 transition-colors">
                          <Pencil size={16} />
                        </button>
      ...
      <Modal isOpen={isModalOpen} onClose={() => !isSubmitting && setIsModalOpen(false)} title={editingTeacher ? "Edit Guru" : "Tambah Guru Baru"} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          {subjects.length === 0 && (
            <div className="p-3 bg-amber-50 border border-amber-200 text-amber-700 text-xs rounded-lg flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="Nama Lengkap" placeholder="Masukkan nama guru" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required />
